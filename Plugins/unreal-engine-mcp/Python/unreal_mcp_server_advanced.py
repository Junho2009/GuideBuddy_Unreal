...
# Enhanced Input asset tools (IA + IMC)
@mcp.tool()
def create_input_action(name: str, value_type: str, save_path: str) -> Dict[str, Any]:
    """Create an InputAction asset at save_path. Idempotent: if asset exists, returns its path.
    value_type: 'bool' | 'axis1d' | 'axis2d'. save_path e.g. /Game/Input/IA_Sprint."""
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    try:
        ...
        return {"success": True, "path": result.get("path", save_path)}
    except Exception as e:
        logger.error(f"create_input_action error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def ensure_input_mapping_context(imc_path: str) -> Dict[str, Any]:
    """Ensure an InputMappingContext asset exists at imc_path; create if missing. Idempotent. Returns path."""
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    try:
        response = unreal.send_command("ensure_input_mapping_context", {"imc_path": imc_path})
        if not response:
            return {"success": False, "message": "No response from Unreal"}
        if response.get("status") == "error":
            return {"success": False, "message": response.get("error", "Unknown error")}
        result = response.get("result") or {}
        return {"success": True, "path": result.get("path", imc_path)}
    except Exception as e:
        logger.error(f"ensure_input_mapping_context error: {e}")
        return {"success": False, "message": str(e)}

@mcp.tool()
def add_mapping_to_imc(imc_path: str, ia_path: str, key: str) -> Dict[str, Any]:
    """Map an InputAction (ia_path) to a key in an InputMappingContext (imc_path). Idempotent: skips if already mapped.
    key: e.g. LeftShift, W, Space."""
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
    try:
        response = unreal.send_command("add_mapping_to_imc", {
            "imc_path": imc_path,
            "ia_path": ia_path,
            "key": key,
        })
        ...
        logger.error(f"add_mapping_to_imc error: {e}")
        return {"success": False, "message": str(e)}
