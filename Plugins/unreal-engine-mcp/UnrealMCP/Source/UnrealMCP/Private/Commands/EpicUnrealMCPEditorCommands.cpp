...
TSharedPtr<FJsonObject> FEpicUnrealMCPEditorCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("new_level_from_template"))
    {
        return HandleNewLevelFromTemplate(Params);
    }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPEditorCommands::HandleSaveCurrentLevel(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        ResultObj->SetStringField(TEXT("level_name"), World->PersistentLevel->GetOutermost()->GetName());
    }
    return ResultObj;
}
...
TSharedPtr<FJsonObject> FEpicUnrealMCPEditorCommands::HandleLoadLevel(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        if (!Params->TryGetStringField(TEXT("level_path"), AssetPath))
        {
            return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'asset_path' parameter"));
        }
    }

    if (!UEditorLevelLibrary::LoadLevel(AssetPath))
    {
...
TSharedPtr<FJsonObject> FEpicUnrealMCPEditorCommands::HandleNewLevelFromTemplate(const TSharedPtr<FJsonObject>& Params)
{
    FString AssetPath;
    if (!Params->TryGetStringField(TEXT("asset_path"), AssetPath))
    {
    ...
    FString TemplateAssetPath = TEXT("/Engine/Maps/Templates/Template_Default");
    ...
    if (UEditorAssetLibrary::DoesAssetExist(AssetPath))
    {
        if (!UEditorLevelLibrary::LoadLevel(AssetPath))
        {
            return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Level '%s' already exists but could not be loaded"), *AssetPath));
        }

        ResultObj->SetBoolField(TEXT("success"), true);
        ResultObj->SetStringField(TEXT("asset_path"), AssetPath);
        ResultObj->SetStringField(TEXT("template_asset_path"), TemplateAssetPath);
        ResultObj->SetBoolField(TEXT("created"), false);
        return ResultObj;
    }

    if (!UEditorLevelLibrary::NewLevelFromTemplate(AssetPath, TemplateAssetPath))
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Failed to create level '%s' from template '%s'"), *AssetPath, *TemplateAssetPath));
    }
