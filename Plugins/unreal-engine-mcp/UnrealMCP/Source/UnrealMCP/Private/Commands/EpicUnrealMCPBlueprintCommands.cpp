...
namespace
{
FString NormalizeBlueprintSavePath(const FString& SavePath)
{
    FString NormalizedPath = SavePath.TrimStartAndEnd();
    ...
    {
        NormalizedPath.LeftChopInline(1, EAllowShrinking::No);
    }
...
UClass* ResolveBlueprintParentClass(const FString& ParentClassName)
{
    ...
    if (ParentClassName.Equals(TEXT("Actor"), ESearchCase::IgnoreCase) || ParentClassName.Equals(TEXT("AActor"), ESearchCase::IgnoreCase))
    {
        return AActor::StaticClass();
    }
    if (ParentClassName.Equals(TEXT("Pawn"), ESearchCase::IgnoreCase) || ParentClassName.Equals(TEXT("APawn"), ESearchCase::IgnoreCase))
    {
        return APawn::StaticClass();
    }
    if (ParentClassName.Equals(TEXT("UserWidget"), ESearchCase::IgnoreCase) || ParentClassName.Equals(TEXT("UUserWidget"), ESearchCase::IgnoreCase))
    {
        return UUserWidget::StaticClass();
    }

    if (ParentClassName.StartsWith(TEXT("/Script/")) || ParentClassName.StartsWith(TEXT("/Game/")))
    {
        if (UClass* DirectClass = LoadClass<UObject>(nullptr, *ParentClassName))
        {
            return DirectClass;
        }
    }

    TArray<FString> Candidates = { ParentClassName };
    if (!ParentClassName.StartsWith(TEXT("A")))
    {
        Candidates.Add(TEXT("A") + ParentClassName);
    }
    if (!ParentClassName.StartsWith(TEXT("U")))
    {
        Candidates.Add(TEXT("U") + ParentClassName);
    }
    ...
    const TArray<FString> Modules = { TEXT("Engine"), TEXT("UMG"), TEXT("CommonUI"), TEXT("UnrealAIPractice") };
    for (const FString& Candidate : Candidates)
    {
        const FString NativeName =
            Candidate.StartsWith(TEXT("A")) || Candidate.StartsWith(TEXT("U")) ?
            Candidate.RightChop(1) :
            Candidate;
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    SavePath = NormalizeBlueprintSavePath(SavePath);

    if (!SavePath.StartsWith(TEXT("/Game/")))
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("'save_path' must start with '/Game/'"));
    }

    if (!UEditorAssetLibrary::DoesDirectoryExist(SavePath) && !UEditorAssetLibrary::MakeDirectory(SavePath))
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to create save directory '%s'"), *SavePath));
    }

    const FString PackagePath = SavePath + TEXT("/") + AssetName;
    ...
    UClass* SelectedParentClass = ResolveBlueprintParentClass(ParentClass);
    if (!SelectedParentClass)
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Could not resolve parent class '%s'"), *ParentClass));
    }
    ...
    if (!UEditorAssetLibrary::SaveLoadedAsset(NewBlueprint, false))
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to save blueprint '%s'"), *PackagePath));
    }

    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetStringField(TEXT("name"), AssetName);
    ResultObj->SetStringField(TEXT("path"), PackagePath);
    ResultObj->SetStringField(TEXT("object_path"), NewBlueprint->GetPathName());
    ResultObj->SetStringField(TEXT("parent_class"), SelectedParentClass->GetPathName());
    ResultObj->SetBoolField(TEXT("is_widget_blueprint"), SelectedParentClass->IsChildOf(UUserWidget::StaticClass()));
    return ResultObj;
}
