...
private:
    ...
    void RegisterBlueprintCompiler();

    void OnAssetRegistryFilesLoaded();

    void InitializeCodeAnalyze();
    ...
    bool bBlueprintCompilerRegistered = false;

    bool bCodeAnalyzeInitialized = false;
};
...
void FPuertsEditorModule::StartupModule()
{
    ...
    Enabled = IPuertsModule::Get().IsWatchEnabled() && !IsRunningCommandlet() && !bStandaloneGameModeRequested;
    ...
    if (Enabled)
    {
        ...
        {
            AssetRegistryFilesLoadedHandle = AssetRegistry.OnFilesLoaded().AddRaw(this, &FPuertsEditorModule::OnAssetRegistryFilesLoaded);
        }
        else
        {
            InitializeCodeAnalyze();
        }
    }
}
...
void FPuertsEditorModule::RegisterBlueprintCompiler()
{
    if (!Enabled || bBlueprintCompilerRegistered)
    {
        return;
    }

    FKismetCompilerContext::RegisterCompilerForBP(UTypeScriptBlueprint::StaticClass(), &MakeCompiler);
    bBlueprintCompilerRegistered = true;
}

void FPuertsEditorModule::OnAssetRegistryFilesLoaded()
{
    if (AssetRegistryFilesLoadedHandle.IsValid())
    {
        if (FAssetRegistryModule* AssetRegistryModule = FModuleManager::Get().GetModulePtr<FAssetRegistryModule>("AssetRegistry"))
        {
...
void FPuertsEditorModule::InitializeCodeAnalyze()
{
    if (!Enabled || bCodeAnalyzeInitialized)
    {
        return;
    }

    // Delay the startup sync until the asset registry finishes loading so
    // UE's blueprint validators are registered before Puerts recompiles TS blueprints.
