...
static bool IsLatentActionInfoProperty(const PropertyMacro* Property)
{
    if (const StructPropertyMacro* StructProperty = CastFieldMacro<StructPropertyMacro>(Property))
    {
        static const FName LatentActionInfoName(TEXT("LatentActionInfo"));
        return StructProperty->Struct && StructProperty->Struct->GetFName() == LatentActionInfoName;
    }

    return false;
}

static bool ShouldSkipMalformedDefaultValue(const PropertyMacro* Property, const FString& DefaultValue)
{
    if (!IsLatentActionInfoProperty(Property))
    {
        return false;
    }

    return DefaultValue.IsEmpty() || DefaultValue == Property->GetName() || !DefaultValue.StartsWith(TEXT("("));
}
            ...
            for (TFieldIterator<PropertyMacro> ParamIt(InFunction); ParamIt; ++ParamIt)
            {
                ...
                if (Property->PropertyFlags & CPF_Parm)
                {
                    if (!(Property->PropertyFlags & CPF_ReturnParm))
                    {
                        ...
                        {
                            if (ShouldSkipMalformedDefaultValue(Property, *DefaultValuePtr))
                            {
                                continue;
                            }
