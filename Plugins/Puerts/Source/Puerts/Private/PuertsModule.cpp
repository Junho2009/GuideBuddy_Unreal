...
public:
    ...
    void HandleDeferredAutoEnable(UWorld* InWorld, const UWorld::InitializationValues InInitializationValues);
...
private:
    ...
    FDelegateHandle DeferredAutoEnableHandle;
};
...
void FPuertsModule::StartupModule()
{
    ...
    const bool bStandaloneGameModeRequested = FParse::Param(FCommandLine::Get(), TEXT("game"));

    if (Settings.AutoModeEnable)
    {
        if (bStandaloneGameModeRequested || IsRunningGame())
        {
            UE_LOG(PuertsModule, Display, TEXT("AutoMode requested during game startup; deferring Enable() until OnPostWorldInitialization."));
            DeferredAutoEnableHandle =
                FWorldDelegates::OnPostWorldInitialization.AddRaw(this, &FPuertsModule::HandleDeferredAutoEnable);
        }
        else
        {
            UE_LOG(PuertsModule, Display, TEXT("AutoMode requested outside game startup; enabling immediately."));
            Enable();
        }
    }
...
void FPuertsModule::HandleDeferredAutoEnable(UWorld* InWorld, const UWorld::InitializationValues InInitializationValues)
{
    if (DeferredAutoEnableHandle.IsValid())
    {
        FWorldDelegates::OnPostWorldInitialization.Remove(DeferredAutoEnableHandle);
        DeferredAutoEnableHandle.Reset();
    }
    ...
    if (!Enabled)
    {
        UE_LOG(PuertsModule,
            Display,
            TEXT("OnPostWorldInitialization reached for world=%s; enabling deferred Puerts AutoMode."),
            InWorld ? *InWorld->GetPathName() : TEXT("<null>"));
        Enable();
    }
}
...
{
    UE_LOG(PuertsModule, Display, TEXT("Enable() called. IsRunningGame=%s"), IsRunningGame() ? TEXT("true") : TEXT("false"));
    Enabled = true;
...
{
    UE_LOG(PuertsModule, Display, TEXT("Disable() called."));
    Enabled = false;
    JsEnv.Reset();
    JsEnvGroup.Reset();
    GUObjectArray.RemoveUObjectCreateListener(static_cast<FUObjectArray::FUObjectCreateListener*>(this));
