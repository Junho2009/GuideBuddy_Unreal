...
type PlayerLevel = "novice" | "intermediate" | "advanced";
...
interface BuildCoachingOptions {
  generatedAt?: string;
...
interface CoachingOutput extends JsonRecord {
  schema_version: string;
  run_id: string;
  generated_at: string;
  source_files: JsonRecord;
  provider: JsonRecord;
  player_level_assumption: JsonRecord;
  llm_input_summary: JsonRecord;
  review_card: JsonRecord;
  practice_objective: JsonRecord;
  drill_spec_candidate: JsonRecord;
  final: JsonRecord;
}
...
const DEFAULT_PLAYER_LEVEL: PlayerLevel = "novice";
let fileSystemModule: FileSystemModule | undefined;
...
export function generateCoachingForRunDirectory(
  runDirectoryInput: string,
  outputPathInput?: string,
  options: BuildCoachingOptions = {}
): CoachingOutput {
  const fs = getFs();
  const path = getPath();
...
export function buildCoaching(
  ...
  const confidence = numberField(finalDiagnosis, "confidence") ?? numberField(deterministicDiagnosis, "confidence") ?? 0;
...
function buildLlmInputSummary(
  ...
  return {
    ...
    duration_seconds: numberField(summary, "duration_seconds") ?? null,
    diagnosis: {
      primary_failure: primaryFailure,
...
function buildPracticeObjective(
  primaryFailure: string,
  playerLevel: PlayerLevel,
  seed: JsonRecord,
...
function buildDrillSpecCandidate(
  primaryFailure: string,
  playerLevel: PlayerLevel,
  practiceObjective: JsonRecord,
  seed: JsonRecord
): JsonRecord {
...
function buildEvidenceLine(primaryFailure: string, confidence: number, signals: JsonRecord[], evidenceSeqs: number[]): string {
  if (signals.length > 0) {
...
function signalSummaries(rawSignals: unknown[]): JsonRecord[] {
  return rawSignals
    .map((value) => recordField(value, ""))
    .filter((value) => Object.keys(value).length > 0)
    .map((signal) => ({
      id: String(signal.id || ""),
      event_seq: numberField(signal, "event_seq") ?? null,
      event_type: String(signal.event_type || ""),
      time_seconds: numberField(signal, "time_seconds") ?? null,
      tag: String(signal.tag || ""),
      actor_role: String(signal.actor_role || ""),
      confidence: numberField(signal, "confidence") ?? null
    }));
}

function templateFor(primaryFailure: string): string {
  if (primaryFailure === "posture_break_into_execution") {
...
function defaultMetrics(primaryFailure: string): JsonRecord[] {
  if (primaryFailure === "posture_break_into_execution") {
    return [
...
function normalizePlayerLevel(value: unknown): PlayerLevel {
...
function promptStrength(confidence: number): string {
  if (confidence >= 0.8) {
    return "direct";
  }
...
function recordField(value: unknown, fieldName: string): JsonRecord {
...
function runCli(argv: string[]): void {
  ...
  let runDirectory = "";
  let outputPath = "";
  let playerLevel: PlayerLevel = DEFAULT_PLAYER_LEVEL;
...
function getFs(): FileSystemModule {
  if (!fileSystemModule) {
    fileSystemModule = require("fs") as FileSystemModule;
  }
