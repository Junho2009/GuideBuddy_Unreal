...
interface AttemptSummary extends JsonRecord {
  schema_version?: string;
  run_id?: string;
  map?: string;
  start_time?: string;
  ...
  end_reason?: string;
  death_info?: JsonRecord | null;
  damage_summary?: JsonRecord;
}
...
interface DiagnosisOutput extends JsonRecord {
  schema_version: string;
  run_id: string;
  generated_at: string;
  source_files: JsonRecord;
  deterministic: JsonRecord;
  llm_review: JsonRecord;
  final: JsonRecord;
}
...
export function generateDiagnosisForRunDirectory(runDirectoryInput: string, outputPathInput?: string): DiagnosisOutput {
  ...
  const summary = readJson(summaryPath) as AttemptSummary;
...
function findTerminalFailure(events: CombatEvent[], summary: AttemptSummary): TerminalFailure | undefined {
...
function classifyTerminalFailure(
  ...
  const playerAttack = findRecentPlayerAttackEvent(evidenceEvents, sourceEvent?.time_seconds ?? terminalTime);
  const dodgeInput = findRecentDodgeInput(evidenceEvents, sourceEvent?.time_seconds ?? terminalTime);
  const hasExecution = Boolean(executionEvent) ||
    tagMatches(String(terminal.source?.tag || ""), ["execut"]) ||
...
function buildInsufficientEvidenceClassification(summary: AttemptSummary): Classification {
...
function selectEvidenceEvents(events: CombatEvent[], terminal: TerminalFailure): CombatEvent[] {
  const terminalTime = terminal.terminalEvent.time_seconds;
...
function selectFallbackEvidenceEvents(events: CombatEvent[]): CombatEvent[] {
  return events
    .filter(isRelevantEvidenceEvent)
    .slice(Math.max(0, events.length - 24));
}

function isRelevantEvidenceEvent(event: CombatEvent): boolean {
...
function buildEvidenceWindow(evidenceEvents: CombatEvent[]): JsonRecord {
...
function buildPracticeObjectiveSeed(
...
function readableFailureInfoZh(primaryFailure: string): { label: string; practiceSuggestion: string } {
...
function confidenceLabelZh(confidence: number): string {
  if (confidence >= 0.8) {
    return "高";
  }
...
function buildReadableEvidenceZh(signals: JsonRecord[], evidenceEvents: CombatEvent[]): string[] {
...
function describeSignalZh(signalRecord: JsonRecord): string {
...
function signalLabelZh(id: string): string {
...
function candidateFocusesFor(primaryFailure: string): string[] {
  if (primaryFailure === "posture_break_into_execution") {
...
function metricsFor(primaryFailure: string): JsonRecord[] {
  if (primaryFailure === "posture_break_into_execution") {
    return [
...
function metric(id: string, unit: string, direction: string): JsonRecord {
  return { id, unit, direction };
}

function signal(id: string, event: CombatEvent, confidence: number): JsonRecord {
  return {
    id,
    event_seq: event.seq,
...
function compactRecords(values: Array<JsonRecord | undefined>): JsonRecord[] {
  return values.filter((value): value is JsonRecord => Boolean(value));
}

function normalizeSource(source: JsonRecord | undefined, evidenceEvents: CombatEvent[]): JsonRecord | null {
  if (!source) {
    const sourceEvent = findRecentEnemyAttackEvent(evidenceEvents, evidenceEvents[evidenceEvents.length - 1]?.time_seconds || 0);
    return sourceEvent ? {
...
function findExecutionEvent(events: CombatEvent[], terminalTime: number): CombatEvent | undefined {
  return events.find((event) =>
    tagMatches(getEventTag(event), ["execut"]) &&
    Math.abs(event.time_seconds - terminalTime) <= 1.0);
}

function findRecentEnemyAttackEvent(events: CombatEvent[], terminalTime: number): CombatEvent | undefined {
...
function findRecentPlayerAttackEvent(events: CombatEvent[], anchorTime: number): CombatEvent | undefined {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
...
function findRecentDodgeInput(events: CombatEvent[], anchorTime: number): CombatEvent | undefined {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
    if (event.time_seconds > anchorTime + 0.2) {
      continue;
...
function isEnemyAttackLike(event: CombatEvent): boolean {
  return tagMatches(getEventTag(event), ["attack", "execut", "riposte", "hit"]);
}

function sourceToEvent(events: CombatEvent[], source: JsonRecord | undefined): CombatEvent | undefined {
  if (!source) {
    return undefined;
  }
...
function findEventBySeq(events: CombatEvent[], seq: number | undefined): CombatEvent | undefined {
  if (seq === undefined) {
    return undefined;
  }
...
function findDeathStateEvent(events: CombatEvent[]): CombatEvent | undefined {
  return events.find((event) =>
    event.event_type === "state_activated" &&
...
function findFatalDamageEvent(events: CombatEvent[]): CombatEvent | undefined {
  return events.find((event) =>
...
function isDeathTag(tag: string): boolean {
...
function tagMatches(value: string, needles: string[]): boolean {
...
function numberField(value: unknown, fieldName: string): number | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function firstString(events: CombatEvent[], fieldName: string): string | undefined {
...
function readJsonl(filePath: string): CombatEvent[] {
  return fs.readFileSync(filePath, "utf8")
    .split(/\r?\n/)
    .filter((line) => line.trim().length > 0)
    .map((line, index) => {
      try {
        return JSON.parse(line) as CombatEvent;
      } catch (error) {
...
function findLatestTelemetryRunDirectory(): string {
