...
interface BuildDrillOptions {
  generatedAt?: string;
  sourceKind?: string;
  randomSeed?: number;
}

interface DrillArtifacts {
  drillSpec: JsonRecord;
  drillSession: JsonRecord;
}

interface NormalizedParameters extends JsonRecord {
  target_error: string;
  focus: string;
...
export function generateDrillForRunDirectory(
...
export function generateDrillForCoachingFile(
  coachingPathInput: string,
  outputDirectoryInput?: string,
  options: BuildDrillOptions = {}
): DrillArtifacts {
  const fs = getFs();
...
export function buildDrillArtifacts(
  outputDirectoryInput: string,
  coachingPathInput: string,
  coaching: JsonRecord,
  options: BuildDrillOptions = {}
...
function validateAndNormalizeCandidate(
  candidate: JsonRecord,
  practiceObjective: JsonRecord,
  final: JsonRecord
  ...
  const enemyCount = integerField(parameters, "enemy_count") ?? 1;
  const repetitionGoal = integerField(parameters, "repetition_goal") ?? 5;
  const successWindowSeconds = numberField(parameters, "success_window_seconds") ?? 1.5;
  const maxConsecutiveFailures = integerField(parameters, "max_consecutive_failures") ?? 3;
...
function validateNoUnknownFields(record: JsonRecord, allowedNames: string[], label: string): void {
...
function scanUnsafeCandidate(value: unknown, pathLabel: string): void {
...
function normalizeSeed(inputSeed: unknown, runId: string, objectiveId: string): number {
...
function difficultyFor(playerLevel: PlayerLevel): number {
...
function integerField(value: unknown, fieldName: string): number | undefined {
...
function findLatestCoachingRunDirectory(): string {
  const fs = getFs();
  const path = getPath();
