...
| `MVP05_DRILL_ARENA_RUNTIME` | 练习场运行时接入 | 根据 MVP04 的 drill specs 生成可进入的针对性练习场，并允许玩家切换到该练习场做单点训练 | 待编译 |
...
5. 再做 `MVP05_DRILL_ARENA_RUNTIME`，让玩家可以从当前战斗切换进入由 `drill_spec.json` 驱动的针对性练习场。
...
`MVP04_ADAPTIVE_DRILL_GENERATION@v0.1` 已通过自动验收，证据见 `aiflow/contracts/runs/MVP04_ADAPTIVE_DRILL_GENERATION.run-001.yaml`。
