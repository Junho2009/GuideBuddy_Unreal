...
在 `SampleDemoShowcaseMap` 中建立最小战斗遥测能力。新增逻辑优先使用 Puerts + TypeScript，让一次玩家尝试能输出可解析的语义事件流和尝试摘要；C++ 只作为必要桥接层。
...
- 新增遥测、事件归一化和数据导出应尽量落在 TypeScript 或结构化文本层。
- C++ 只负责 Puerts 启动、TempestCombatFramework delegate 转发、缺失反射接口暴露等薄桥接。

## 待确认点

1. MVP01 是否允许新增 `Content/JavaScript/GuideBuddy/` 作为 GuideBuddy TypeScript 代码入口？
...
- 允许新增 GuideBuddy TypeScript 遥测模块，并在必要时新增极薄 C++ 桥接组件或子系统。
