...
在运行时接入方面，MVP02 新增 `F10` 指导按钮路径。玩家按下后，GuideBuddy 会发出 `guide_request` 信号，并在当前 run 下生成 `guide_requests/request-001/` 快照，包含 `combat_events.jsonl`、`attempt_summary.json` 和 `diagnosis.json`。
...
- 当前运行时触发是 `F10` 快捷键，还不是正式 UI 按钮；后续可在 MVP03/MVP04 产品化为屏幕按钮或手柄映射。
- 当前保存状态提示使用 UE on-screen debug message，后续可替换为正式 HUD / UMG 状态组件。
